...
From the graph, we see that lines are almost overlapped, and $R^2$ are very close to the ones from original data. 
...
```
We tried three version of this model, we found that model $y_t=b_1y_{t-12}b_4y_{t-48}+b_5insurance + b_6SUVs$ makes better prediction for year 2019. MSE = `r MSE` and $R^2$ = `r (cor(predictions$sales, predictions$pred))^2`
